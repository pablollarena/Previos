...
class PDF extends FPDF{
    ...
    function ImprovedTabla($header,$header2,$header3,$data){
        $w = array(1.2,1.9,1.3,1.6,1.4,1.4,1.4,1.4,1.4,1.4,1.4,1.4,1.4);
       ...
       // $z = array(1.2,1,1,1,1,0.8,1,1,1.5,9.2);
        ...
        foreach ($data as $vRow){
            ...
            $this->Cell($w[2],0.7,$vRow->getConten()->getPesoCarga(),1,0,'LR');
            $this->Cell($w[3],0.7,$vRow->getConten()->getCantidadBultos(),1,0,'LR');
            $this->Cell($w[4],0.7,$vRow->getConten()->getBultosDañados() == 1 ? 'SI' : 'NO',1,0,'LR');
            $this->Cell($w[5],0.7,$vRow->getConten()->getCantBultDañados(),1,0,'LR');
            $this->Cell($w[6],0.7,$vRow->getConten()->getPalletsMadera() == 1 ? 'X': '',1,0,'LR');
            $this->Cell($w[7],0.7,$vRow->getConten()->getSacos() == 1 ? 'X': '',1,0,'LR');
            $this->Cell($w[8],0.7,$vRow->getConten()->getHuacalesMadera() == 1 ? 'X': '',1,0,'LR');
            $this->Cell($w[9],0.7,$vRow->getConten()->getPalletsPlastico() == 1 ? 'X': '',1,0,'LR');
            $this->Cell($w[10],0.7,$vRow->getConten()->getSuperBolsas() == 1 ? 'X': '',1,0,'LR');
            $this->Cell($w[11],0.7,$vRow->getConten()->getCajasMadera() == 1 ? 'X': '',1,0,'LR');
            $this->Cell($w[12],0.7,$vRow->getConten()->getCartonada() == 1 ? 'X': '',1,0,'LR');
            ...
            $this->MultiCell($x[5],0.3,$vRow->getConten()->getOtros(),1,1);
...
$header = array('IMO','Peso','PCarga','CantBultos','BDañados','CDañados','PMadera','Sacos','HMadera','PPlastico','SBolsas','CMadera','Cartonada');
